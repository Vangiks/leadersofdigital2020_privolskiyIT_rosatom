...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СкрываемыеПрофили = Новый Массив;
	СкрываемыеПрофили.Добавить(Справочники.ПрофилиГруппДоступа.Администратор);
	СкрываемыеПрофили.Добавить(Справочники.ПрофилиГруппДоступа.ПоставляемыйПрофильПоИдентификатору(УправлениеДоступомСлужебный.ОписаниеПрофиляОткрытиеВнешнихОтчетовИОбработок().Имя));
	СкрываемыеПрофили.Добавить(Справочники.ПрофилиГруппДоступа.ПоставляемыйПрофильПоИдентификатору(УправлениеДоступомПереопределяемый.ОписаниеПрофиля_БазовыеПрава().Имя));
	
	Список.Параметры.УстановитьЗначениеПараметра("СкрываемыеПрофили", СкрываемыеПрофили);
	...
	УстановитьОтборСкрываемыеГруппы(Список, Элементы.ПоказатьВсе.Пометка);
...
Процедура ПоказатьВсе(Команда)
	...
	Элементы.ПоказатьВсе.Пометка = Не Элементы.ПоказатьВсе.Пометка;
...
&НаКлиенте
Процедура ОбновитьГруппыДоступаПоОрганизациям(Команда)
	
	ЕстьОшибки = Ложь;
	ОбновитьГруппыДоступаПоОрганизациямНаСервереБезКонтекста(ЕстьОшибки);
	
	ТекстСообщения = ?(ЕстьОшибки,
		НСтр("ru = 'При обновлении возникли ошибки (см. сообщения)'"),
		НСтр("ru = 'Обновление выполнено успешно'"));
...
Процедура УстановитьОтборСкрываемыеГруппы(Список, Пометка)
	
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(
		Список, "Скрыть", Ложь,,, Не Пометка);
	
КонецПроцедуры

&НаСервереБезКонтекста
Процедура ОбновитьГруппыДоступаПоОрганизациямНаСервереБезКонтекста(ЕстьОшибки)
	Справочники.ГруппыДоступа.ОбновитьГруппыДоступаПоОрганизациям(ЕстьОшибки);
КонецПроцедуры
