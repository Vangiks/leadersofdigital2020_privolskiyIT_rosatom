...
Процедура ОбновитьГруппыДоступаПоОрганизациям(ЕстьОшибки) Экспорт
	...
	Запрос.Текст =
		...
		|	И ГруппыДоступа.ЗначенияДоступа.ВидДоступа = ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка)
		|	И ГруппыДоступа.Профиль = &ПрофильФЛ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ЕСТЬNULL(втГруппыДоступа.Ссылка, ЗНАЧЕНИЕ(Справочник.ГруппыДоступа.ПустаяСсылка)) КАК ГруппаДоступа
		|ИЗ
		|	Справочник.Организации КАК Организации
		|		ЛЕВОЕ СОЕДИНЕНИЕ втГруппыДоступа КАК втГруппыДоступа
		|		ПО Организации.Ссылка = втГруппыДоступа.ЗначениеДоступа
		|ГДЕ
		|	НЕ Организации.ПометкаУдаления
		|	И ИСТИНА В (втГруппыДоступа.Ссылка ЕСТЬ NULL, втГруппыДоступа.Наименование <> Организации.Наименование)";
	
	ПрофильФЛ = Справочники.ПрофилиГруппДоступа.ПоставляемыйПрофильПоИдентификатору(
		УправлениеДоступомПереопределяемый.ОписаниеПрофиля_ПользовательФЛ().Имя);
	
	Запрос.УстановитьПараметр("ПрофильФЛ", ПрофильФЛ);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ГруппаДоступа.Пустая() Тогда
			...
			ГруппаОбъект.Профиль = ПрофильФЛ;
			
			стрВидыДоступа = ГруппаОбъект.ВидыДоступа.Добавить();
			стрВидыДоступа.ВидДоступа = Справочники.Организации.ПустаяСсылка();
			
			стрЗначенияДоступа = ГруппаОбъект.ЗначенияДоступа.Добавить();
			стрЗначенияДоступа.ВидДоступа      = Справочники.Организации.ПустаяСсылка();
			стрЗначенияДоступа.ЗначениеДоступа = Выборка.Организация;
		...
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				СтрШаблон(НСтр("ru = 'Не удалось записать группу доступа по организации %1 по причине: %2'"), Выборка.Организация, ОписаниеОшибки()),,,, ЕстьОшибки);
