...
Функция ЗагрузитьКурсыВалютПоПараметрам(Знач Валюты, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки, ПриЗагрузкеВозниклиОшибки = Ложь)
	...
	УстановитьПривилегированныйРежим(Истина);
	ИспользоватьАльтернативныйСервер = Константы.ИспользоватьАльтернативныйСерверДляЗагрузкиКурсовВалют.Получить();
	ИспользоватьАльтернативныйСерверЦБДляЗагрузкиКурсовВалют = 
		Константы.ИспользоватьАльтернативныйСерверЦБДляЗагрузкиКурсовВалют.Получить();
	УстановитьПривилегированныйРежим(Ложь);
		
	Если ИспользоватьАльтернативныйСерверЦБДляЗагрузкиКурсовВалют Тогда
		
		ПараметрыПодключенияWSПрокси = ОбщегоНазначения.ПараметрыПодключенияWSПрокси();
		
		ПараметрыПодключенияWSПрокси.АдресWSDL = "http://www.cbr.ru/DailyInfoWebServ/DailyInfo.asmx?WSDL";
		ПараметрыПодключенияWSПрокси.URIПространстваИмен = "http://web.cbr.ru/";
		ПараметрыПодключенияWSПрокси.ИмяСервиса = "DailyInfo";
		ПараметрыПодключенияWSПрокси.ИмяТочкиПодключения = "DailyInfoSoap";
		
		Прокси = ОбщегоНазначения.СоздатьWSПрокси(ПараметрыПодключенияWSПрокси);
		
		ТипWSПараметра = Прокси.ФабрикаXDTO.Пакеты.Получить("http://web.cbr.ru/").Получить("GetCursOnDate");
		WSПараметр  = Прокси.ФабрикаXDTO.Создать(ТипWSПараметра);
		WSПараметр.On_Date= НачалоПериодаЗагрузки;
		
		КурсыВалют = Прокси.GetCursOnDate(WSПараметр);
		ТаблицаКурсовВалют = КурсыВалют.GetCursOnDateResult.diffgram.ValuteData.ValuteCursOnDate;
	...
	Для Каждого Валюта Из Валюты Цикл
		...
		Иначе
			...
			Если ИспользоватьАльтернативныйСерверЦБДляЗагрузкиКурсовВалют Тогда
				...
				ПоясняющееСообщение = ЗагрузитьКурсВалютыИзЦБ(Валюта.Валюта, ТаблицаКурсовВалют, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки) + Символы.ПС;
...
Функция ЗагрузитьКурсВалютыИзЦБ(Знач Валюта, Знач КурсыВалют, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки)
	
	РеквизитыВалюты = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
		Валюта, "Наименование, Код");	
	
	Для Каждого Строка Из КурсыВалют Цикл
		
		Если РеквизитыВалюты.Код = Строка.Vcode
			И РеквизитыВалюты.Наименование = Строка.VChCode Тогда
		
			НаборЗаписей = РегистрыСведений.КурсыВалют.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Валюта.Установить(Валюта);
			НаборЗаписей.Отбор.Период.Установить(ОкончаниеПериодаЗагрузки);
			
			Запись = НаборЗаписей.Добавить();
			Запись.Валюта = Валюта;
			Запись.Период = ОкончаниеПериодаЗагрузки;
			Запись.Курс = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Строка.Vcurs);
			Запись.Кратность = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(Строка.Vnom);
