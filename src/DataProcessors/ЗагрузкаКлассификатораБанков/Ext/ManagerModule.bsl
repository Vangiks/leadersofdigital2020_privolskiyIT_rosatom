...
Процедура ЗагрузитьКлассификаторыБанковЦБ() Экспорт
	
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ЗагрузкаКлассификаторовБанков);
	
	ИмяСобытия = НСтр("ru = 'Банки.Загрузка классификаторов банков'", ОбщегоНазначения.КодОсновногоЯзыка());
	
	ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru = 'Начата регламентная загрузка классификаторов банков'"));

	ЗагрузитьКлассификаторыБанковЦБИзФайла();
...
Процедура ЗагрузитьКлассификаторыБанковЦБИзФайла() Экспорт
	
	Результат = ПолучениеФайловИзИнтернета.СкачатьФайлНаСервере("http://cbr.ru/scripts/XML_bic2.asp");
	
	Если Результат.Статус Тогда
		ЗагрузитьДанныеИзФайлаЦБ(Результат.Путь);
	КонецЕсли;
...
Процедура ЗагрузитьДанныеИзФайлаЦБ(ПутьКФайлу)
	...
	СведенияОБанках = Новый ТаблицаЗначений;
	СведенияОБанках.Колонки.Добавить("БИК");
	СведенияОБанках.Колонки.Добавить("Наименование");
	
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.ОткрытьФайл(ИмяФайла);
	Пока ЧтениеXML.Прочитать() Цикл
		
		Если ЧтениеXML.ЛокальноеИмя = "Record"
			И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			НоваяСтрока = СведенияОБанках.Добавить();
		КонецЕсли;
		
		
		Если ЧтениеXML.ЛокальноеИмя = "ShortName"
			И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			ЧтениеXML.Прочитать();
			НоваяСтрока.Наименование = ЧтениеXML.Значение;
		КонецЕсли;
		
		Если ЧтениеXML.ЛокальноеИмя = "Bic"
			И ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			ЧтениеXML.Прочитать();
			НоваяСтрока.БИК = СокрЛП(ЧтениеXML.Значение);
		КонецЕсли;
	...
	ФайловаяСистема.УдалитьВременныйФайл(ПутьКФайлу);
	
	Для Каждого СведениеОБанке Из СведенияОБанках Цикл
		ЗаписатьЭлементКлассификатораБанковРФЦБ(СведениеОБанке);	
	КонецЦикла;
...
Процедура ЗаписатьЭлементКлассификатораБанковРФЦБ(СведенияОБанке)
